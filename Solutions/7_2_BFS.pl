...
goal([1,2,3,
       4,0,5,
       6,7,8]).
%%"EXAMPLE INPUT: bfs([1,2,3,0,4,5,6,7,8],Ans,Len). "
bfs(S,Ans,Len):-
     bfsacc(S,Ans,[],[]),len(Ans,Len).

bfsacc(S,[S|Path],Queue,Path):-goal(S),!.
bfsacc(S,Path,Queue,Checked):-
     not(our_member(S,Checked)),
     findall(S2,move(S,S2),Next),
     append(Queue,Next,[Val|Queue1]),    
     bfsacc(Val,Path,Queue1,[S|Checked]).

bfsacc(S,Path,[Val|Queue],Checked):-
     bfsacc(Val,Path,Queue,Checked).
